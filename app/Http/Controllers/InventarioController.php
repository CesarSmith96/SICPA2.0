...
class InventarioController extends Controller {
	...
	*/
	...
	public function postIndex(Request $request)
	{
		...
		if(Input::get('imprimirinv'))
		return view('reporte.inventario',['inventarios'=> $inventarios]);
	}
		...
		foreach ($inventarios as $inventario) {
			...
			foreach ($detalles as $detalle) {
				...
				if ($um_producto != $um_detalle) {
					if((Conversion::where('um_id1',$um_producto)->where('um_id2',$um_detalle)->count())==0 && (Conversion::where('um_id2',$um_producto)->where('um_id1',$um_detalle)->count())==0)
					{
						continue;
					}
	...
	public function getVerificar()
	{
		...
		$registros = array(array("PRODUCTO","UNIDADES"));
		...
		foreach ($inventarios as $inventario) {
			...
			foreach ($detalles as $detalle) {
				...
				if ($um_producto != $um_detalle) {
					if((Conversion::where('um_id1',$um_producto)->where('um_id2',$um_detalle)->count())==0 && (Conversion::where('um_id2',$um_producto)->where('um_id1',$um_detalle)->count())==0)
					{
						array_push($registros,array($detalle->unidadproducto->producto->prod_desc,$detalle->unidadproducto->unidadmedida->um_desc));
						array_push($registros,array($detalle->unidadproducto->producto->prod_desc,$detalle->unidadproducto->producto->unidadmedida->um_desc));
					}
				}
