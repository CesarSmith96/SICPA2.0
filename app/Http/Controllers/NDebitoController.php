...
class NDebitoController extends Controller {
	...
	*/
	...
	{
		$comprobantes = Comprobante::join('t_operacion','t_operacion.comp_id','=','t_comprobante.comp_id')->select('t_comprobante.*')->where('t_operacion.tope_id','=','6')->where('t_comprobante.comp_id','<>','1')->orderBy('comp_fecha','desc')->orderBy('comp_nro','desc')->get();
		...
		// 3 Nota de Debito
		return view('ndebito.mostrar',['comprobantes'=> $comprobantes,'tipocomprobanteincs'=> $tipocomprobanteincs,'entidades'=> $entidades,'vendedores'=> $vendedores]);
	}
	...
	public function getCrear(Request $request)
	{
		...
		$tipocomprobanteincs = TipoComprobanteInc::where('tcomp_id',4)->where('tcompinc_cod','<>','05')->where('tcompinc_cod','<>','08')->where('tcompinc_cod','<>','09')->get();
		$vendedores = Vendedor::orderBy('vend_nom','asc')->get();
		return view('ndebito.crear',['tipocomprobanteincs'=> $tipocomprobanteincs,'entidades'=> $entidades,'vendedores'=> $vendedores,'comprobante'=> $comprobante]);
		return $comp_id;
	}
	...
	public function postCrear(CrearNotaCreditoEmitidaRequest $request)
	{
		...
		(
			[
				'tope_id' => 6, ///// tipo operacion nota de credito recibida
				'comp_id' => $comp_id,
				'ie_id' => 1 ///// para ie RESGUARDO
			]
		);

		return redirect("/validado/ndebito")->with('creado','Nota de Débito creada');
	...
	public function getEditar(Request $request)
	{
		...
		return view('ndebito.editar',['comprobante'=>$comprobante,'comprobante_ref'=> $comprobante_ref,'tipocomprobanteincs'=> $tipocomprobanteincs,'entidades'=>$entidades,'vendedores'=> $vendedores]);
	...
	public function postEditar(EditarNotaCreditoEmitidaRequest $request)
	{
		...
		return redirect('/validado/ndebito')->with('actualizado','Comprobante actualizado');
	}
	...
	public function getEliminar(Request $request)
	{
		...
		return redirect('/validado/ndebito')->with('eliminado','Comprobante eliminado');
	}
