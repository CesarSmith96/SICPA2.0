...
class ReporteController extends Controller {
	...
	*/
	...
	public function getIndex(Request $request)
	{
		...
		$funcionarios = Vendedor::orderBy('vend_nom','asc')->where('vend_tipo','funcionario')->get();
		...
		return view('reporte.mostrar',['tipocomprobantes'=> $tipocomprobantes,'vendedores'=> $vendedores,'entidades'=> $entidades,'clientes'=> $clientes,'productos'=> $productos,'tipoccs'=> $tipoccs,'tipogastos'=> $tipogastos,'funcionarios'=>$funcionarios]);
	}
	...
	public function postDetallegastos(Request $request)
	{
		...
		$detalleieexternos=$detalleieexternos->orderBy('ie_fecha','asc')->orderBy('ie_comp','asc')->get();
...
public function postResumen(Request $request)
	...
	public function postCredito()
	{
		$comprobantes = Comprobante::join('t_operacion','t_operacion.comp_id','=','t_comprobante.comp_id')->select('t_comprobante.*')->where('t_operacion.tope_id','=','2')->where('t_comprobante.comp_id','<>','1')->where('t_comprobante.comp_saldo','>','0')->where('t_comprobante.comp_cond','=','AL CREDITO')->orderBy('comp_fecha','desc')->orderBy('comp_nro','desc')->get();
		$vendedores = Vendedor::orderBy('vend_nom','asc')->where('vend_tipo','=','VENDEDOR')->get();
		...
		return view('reporte.credito',['comprobantes'=> $comprobantes,'tipocomprobantes'=> $tipocomprobantes,'entidades'=> $entidades,'vendedores'=> $vendedores]);
